...
| Price vs local €/m² | `relvalue` | 9% | financial | **Computed (v4).** The objective price signal. `premium = price / WOZ`, centred on the property's **city median** premium (Amstelveen vs Amsterdam; falls back to the all-set median if a city has <8 priced units). Score = `clamp(1..10, 6 − (premium/cityMedian − 1) × 14)` → neutral 6 at the city median, cheaper-vs-local-peers scores higher. WOZ is used only as the location/size normaliser (price/WOZ ≈ €/m² ÷ local fair €/m²). Null price or WOZ → unscored (renormalise). |
...
| Running costs | `costs` | 8% | financial | **Computed (v4: now includes WOZ taxes).** Full owner monthly = `vve_costs` + `heating_advance` + **WOZ-driven owner taxes** `wozTaxMonthly` = (WOZ × OZB rate + WOZ × 0.35% forfait × 37% marginal) ÷ 12, where OZB ≈ 0.063% Amstelveen / 0.058% Amsterdam (per-gemeente `OZB_RATE`; `MARGINAL_RATE=0` gives OZB-only). Banded ≤160 → 10 · ≤210 → 9 · ≤260 → 8 · ≤320 → 7 · ≤380 → 6 · ≤450 → 5 · ≤540 → 4 · >540 → 3. The "VvE/mo" column still shows VvE+heating only; the **score** uses the tax-inclusive total. Null `vve_costs` → unscored. |
...
Per-property model fields (all optional unless noted): `scores: { value, condition, legal }` (required trio — note `value` is now the condition-adjusted judgment overlay, not raw €/m²), `age_restricted` (bool), `family_adj` + `family_adj_reason`, `location_adj` (−1/+1), `energy_upgrade` ("easy"|"moderate"), `heating_advance` (€/mo). `relvalue` is computed from `price` + `woz` (no field to set); OZB/forfait constants live in `build.js` (`OZB_RATE`, `FORFAIT_RATE`, `MARGINAL_RATE`).
