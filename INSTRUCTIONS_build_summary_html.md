...
- `scores`: `{ value, condition, legal }` each 1–10 (hand-scored). All other criteria (`relvalue, family, location, energy, tenure, costs, outdoor`) are **computed by `build.js`** — see "Scoring model v4". Optional model fields: `age_restricted`, `family_adj` (+`family_adj_reason`), `location_adj`, `energy_upgrade`, `heating_advance`.
...
## Scoring model v4 (must match the artifact and the agents)

Weighted total out of 10, tuned for the buyer's real brief: a **live-in home for one adult + a 3-yo (shared custody), to be sold in 5–10 years**. Two clusters — livability ~51%, financial/resale ~49%.

**v3 (2026-06-09): six criteria are COMPUTED by `build.js` from measured fields — do not hand-score them.** Only `value`, `condition` and `legal` are hand-scored in `scores`. A hand value on a computed key acts as an override and triggers an `OVERRIDE` build warning — use only with a documented reason.

**v4 key change (2026-06-28): the price signal is rebalanced so €/m² outweighs the raw WOZ premium.** A new COMPUTED `relvalue` (9%) — the asking/WOZ premium centred on the property's **city median** — carries the objective "price vs local €/m²" judgment (WOZ is only the location/size-aware normaliser, since price/WOZ ≈ €/m² ÷ local "fair" €/m²; centring cancels the shared peildatum lag). `value` (manual) is trimmed 16→10% and **re-scoped to a condition-adjusted judgment overlay** (over-priced-for-condition, bidding-war temper) — its objective €/m² job now lives in `relvalue`. `costs` now also includes the **WOZ-driven owner taxes** (OZB per gemeente + eigenwoningforfait drag), so a higher WOZ correctly raises carrying cost. Existing hand `value` scores predate this rubric and should be revisited on next review.

| Criterion | key | weight | cluster | How it's scored (1–10) |
|---|---|---|---|---|
| Family fit & space | `family` | 17% | living | **Computed.** 55+/ballotage (`age_restricted: true`) → 1. Studio → 2 · 1 bed → 3 · 2 bed: <55 m² → 4, 55–64 → 6, 65–74 → 7, 75–84 → 8, ≥85 → 9 · 3+ bed: <70 → 8, 70–84 → 9, ≥85 → 10. Plus optional `family_adj` (± int, with `family_adj_reason`) for documented potential, e.g. attic = +1, "2nd bedroom must be created" = −2. |
| Price vs local €/m² | `relvalue` | 9% | financial | **Computed (v4; v5: pinned baseline).** The objective price signal. `premium = price / WOZ`, centred on the **pinned per-city median** in `REL_BASELINE` in `build.js` (v5: pinned per quarter — e.g. 2026-Q3: Amsterdam 1.0076 · Amstelveen 1.0760 — so scores don't drift as listings enter/leave the set; `build.js` warns `BASELINE-DRIFT` when the dynamic median moves >3% off the pin → re-pin at quarter start). Falls back to the dynamic city median (≥8 priced units, else all-set) if no pin. Score = `clamp(1..10, 6 − (premium/baseline − 1) × 14)` → neutral 6 at the baseline, cheaper-vs-local-peers scores higher. WOZ is used only as the location/size normaliser (price/WOZ ≈ €/m² ÷ local fair €/m²). Null price or WOZ → unscored (renormalise). |
| Value (judgment) | `value` | 10% | financial | **Manual.** A condition-adjusted **judgment overlay**, not the raw €/m² (that is now `relvalue`). Start ~6 (fairly priced for its condition); lift to 8–9 when it's a clear bargain for the state it's in or a motivated/zero-competition seller; cut to 3–4 when over-priced **for its condition** or styled to bait a bidding war above a low ask. Use to express what the computed `relvalue` can't see (interior quality vs price, sale dynamics). |
| Location & commute | `location` | 15% | living | **Computed.** Emmakade: ≤7 min → 10, then −0.6/min (floor 2); Zuidas: ≤6 min → 10, then −0.5/min (floor 2); blend **70/30 (v5**, buyer's call 2026-07-06; was 60/40). Plus `location_adj` ∈ {−1, +1} for neighbourhood (quiet/green +1, busy arterial −1). |
| Condition / move-in ready | `condition` | 13% | living | **Manual.** Fully renovated/move-in → 9; modernised → 7–8; dated → 4–5; full gut needed → 2–3. |
...
| Tenure / erfpacht | `tenure` | 7% | financial | **Computed** from the exact `ground` string via `TENURE_SCORE` in `build.js` (eigen grond 10 · eeuwigdurend afgekocht 9 · eigen grond unverified 8 · afgekocht fixed-term 7 · vastgeklikt/afkoop aangevraagd 5 · lopend/tijdvak 4 · erfpacht te verifieren/onbekend 3). **Every distinct `ground` value needs a `TENURE_SCORE` entry** (same discipline as `GROUND_RU`); unmapped → `TENURE-UNMAPPED` warning + renormalise. Null ground → unscored. |
| Running costs | `costs` | 8% | financial | **Computed (v4: WOZ taxes; v5: canon + VvE-health).** Full owner monthly = `vve_costs` + `heating_advance` + **`canon_month`** (erfpacht canon €/mo, 0 when afgekocht — a lopend canon is a real carrying cost, not just a tenure penalty) + **WOZ-driven owner taxes** `wozTaxMonthly` = (WOZ × OZB rate + WOZ × 0.35% forfait × 37% marginal) ÷ 12, where OZB ≈ 0.063% Amstelveen / 0.058% Amsterdam (per-gemeente `OZB_RATE`; `MARGINAL_RATE=0` gives OZB-only). Banded ≤160 → 10 · ≤210 → 9 · ≤260 → 8 · ≤320 → 7 · ≤380 → 6 · ≤450 → 5 · ≤540 → 4 · >540 → 3. **v5 VvE-health cap:** `vve_costs` < €100/mo without `vve_reserve_ok: true` (reserve/MJOP checked) caps the score at 7 — a suspiciously low fee signals an underfunded reserve (special-assessment risk), not a saving; `build.js` warns `VVE-LOW`. The "VvE/mo" column still shows VvE+heating only; the **score** uses the full total. Null `vve_costs` → unscored. |
...
Livability = 17+15+13+6 = 51%; financial = 9+10+10+8+7+5 = 49%.
...
**v5 veto gates (2026-07-06) — dealbreakers don't average away.** After the weighted total, `gatedTotal()` caps the score at **5.4** (red zone) when any gate triggers: manual `legal` < 4 · `lib_zone` set (Schiphol LIB noise/no-build contour, e.g. `"LIB-3"` — check for every Amstelveen shortlist property) · `price` > **€550k** (buyer's hard budget, `BUDGET_CAP`) · `age_restricted`. The pill shows ⛔ with the raw score + reasons in the tooltip; `build.js` warns `GATED …`. Rationale: a weighted mean lets 17% family-fit compensate an unfinanceable legal defect — it shouldn't.

**v5 value-rubric worklist:** hand `value` scores written before the v4 re-scoping distort 10% of the total. `build.js` warns `VALUE-RUBRIC` for any top-15 active property whose `value` isn't re-scored and marked **`value_v4: true`**. Work the list down; don't bulk-set the flag without actually re-scoring.

Per-property model fields (all optional unless noted): `scores: { value, condition, legal }` (required trio — note `value` is now the condition-adjusted judgment overlay, not raw €/m²), `age_restricted` (bool), `family_adj` + `family_adj_reason`, `location_adj` (−1/+1), `energy_upgrade` ("easy"|"moderate"), `heating_advance` (€/mo), **`canon_month` (€/mo, v5)**, **`vve_reserve_ok` (bool, v5)**, **`lib_zone` (string, v5)**, **`value_v4` (bool, v5)**. `relvalue` is computed from `price` + `woz` (no field to set); OZB/forfait constants and `REL_BASELINE`/`BUDGET_CAP` live in `build.js` (`OZB_RATE`, `FORFAIT_RATE`, `MARGINAL_RATE`).
...
2. **Weights legend**: the ten criteria with their percentages, plus a one-line cluster note (livability ~51% / financial ~49%).
...
- Each row's displayed `total` equals the recomputed weighted score (diff < 0.001) — or its **gated cap** (5.4) when a v5 gate triggered (⛔ pill shows the raw score in its tooltip).
